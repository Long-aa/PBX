	...
	<ul class="nav nav-tabs dashboards" role="tablist" id="all_dashboards">
		<?php if (!empty($user_dashboards)) { ?>
			<?php foreach ($user_dashboards as $dashboard_info) { ?>
				<li class="<?php echo ($dashboard_info["id"] == $active_dashboard) ? 'active' : '' ?> menu-order dashboard-menu"
					data-id="<?php echo $dashboard_info["id"] ?>">
					<a data-dashboard
						class="<?php echo ($dashboard_info["id"] == $active_dashboard) ? 'pjax-block' : '' ?>"><?php echo $dashboard_info["name"] ?></a>
	...
	<ul class="nav navbar-nav align-self-baseline" id="side_bar_content">
		...
		<?php if (!empty($user_small_widgets)) { ?>
			<?php foreach ($user_small_widgets as $small_widget) { ?>
				<?php
				$regenuuid = '';
				if (!preg_match('/^[0-9A-F]{8}-[0-9A-F]{4}-4[0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i', (string) $small_widget['id'])) {
				...
				<li class="custom-widget" data-widget_id="<?php echo $small_widget['id']; ?>"
					...
					<a href="#" title="<?php echo $small_widget['rawname'] . " " . $small_widget['widget_type_id']; ?>"
						...
						data-icon="<?php echo $small_widget['icon']; ?>">
...
<div class="side-menu-widgets-container">
	<?php if (!empty($user_small_widgets)) { ?>
		<?php foreach ($user_small_widgets as $small_widget) { ?>
			<div class="widget-extra-menu" id="menu_<?php echo $small_widget['id']; ?>"
				data-id="<?php echo $small_widget['id']; ?>" data-widget_type_id="<?php echo $small_widget['widget_type_id']; ?>"
				data-module="<?php echo $small_widget['rawname']; ?>" data-name="<?php echo $small_widget['name'] ?>"
				...
				<h5 class="small-widget-title"><i class="fa <?php echo $small_widget['icon'] ?>"></i> <span>
						<?php echo $small_widget['widget_name'] ?>
					</span> <small>(
						<?php echo $small_widget['name'] ?>)
					</small></h5>
				<div class="small-widget-content">
				</div>
				<button type="button" class="btn btn-xs btn-danger remove-small-widget"
					data-widget_id="<?php echo $small_widget['id']; ?>"
					data-widget_rawname="<?php echo $small_widget['rawname']; ?>"><?php echo _("Remove Widget") ?></button>
			</div>
		<?php } ?>
	<?php } ?>
</div>
